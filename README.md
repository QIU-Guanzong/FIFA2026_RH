...
其余 48 队均 <0.3pp。**东道主加成**=不可 OOS 验证的假设项，默认关闭，仅作敏感性（+75 Elo → 美/墨/加出线
+4~7.5pp，`--host-boost` 可看）。**符号教训**：曾据 #6 笔记臆测"AFC/CAF 仍偏高"，实测在多趟 Elo 后恰相反
...
**后置（下一轮）**
- **#5 Part B（阵容/首发层）= 数据门槛**：无可靠免费的预测首发/伤停源，付费源待 CEO 决策（明确后置，非遗漏）
