...
（上一趟终值作下一趟先验，把后期跨洲际比赛信息反向传播到早期），在**无泄漏 walk-forward**（2006 至今，
~19,400 场持出国际赛，min_train=200, refit_every=50）上 log loss 单调下降（固定 `goals_scale=0.0018`，passes=4，
log loss **0.9049**, ECE 0.0213）；排名上 Portugal/Netherlands/Germany 上移、Japan/Morocco/Ecuador 下移（Top3 不变）。
残留通胀仍存（需洲际标签/SOS 根治 → #6b）。
...
> 精确口径（2026-06-10 更新）：上述 OOS 数值出自 **Elo-prior 回测预测器**，固定 `goals_scale=0.0018`，
> passes=4，在全量国际赛（~19,400 场持出）上测得 log loss **0.9049**、ECE 0.0213（校准最好）。
> 部署模型用自适配 `goals_scale`（`"auto"` = 0.35/(2·sd)，top-48 sd≈174 → scale≈0.001），
> 是不同的 Elo→λ 映射；在同数据集以 goals_scale=0.001 测得 log loss 0.9172、ECE 0.0597。
> 用 `goals_scale="auto"` 在**全量 319 队**回测则为 0.9525（不可比：全量 Elo 跨度约 top-48 两倍，scale 被压到 0.00051）。
> 部署仅预测 top-48 强队间对决，0.9049 是最接近的诚实参照，而非精确端到端准确度。
...
**已接入（评分增强，#6 完成）**
- 多趟暖启动 Elo：在国际赛无泄漏 walk-forward 上（~19,400 场持出，passes=4，goals_scale=0.0018）OOS log loss **0.9049**（ECE 0.0213），洲际通胀显著下降
